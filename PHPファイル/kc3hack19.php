...
if (file_exists($jsonFilePath)) {
    ...
    if (!empty($jsonString)) {
        ...
        $existingData = array("objects" => array()); //空の配列
    }



    // 新しいデータを追加.  objectsキーの中に追加するように変更
    if (isset($existingData['objects']) && is_array($existingData['objects'])) {
        $existingData['objects'][] = $newData;  // 既存の配列に追加
    } else {
        //objects キーが存在しないか、配列でない場合。
        http_response_code(500);
        echo json_encode(array("error" => "Invalid JSON structure in existing file.  'objects' key is missing or not an array."));
        exit;
    }
} else {
    // ファイルが存在しない場合は、新しい配列を作成
    $existingData = array("objects" => array($newData));  // 新しいデータを含む配列
}
